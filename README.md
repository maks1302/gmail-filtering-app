...
AI is available as a rule type rather than as a global inbox classifier. Provider-wide controls live in **Settings → AI Settings**:

- OpenRouter API key, primary model, optional verifier model, and optional regex-helper model
- global dry-run mode
...
Pattern conditions include an **AI Regex Helper** sparkle button while creating or editing a rule. Use it to generate, improve, fix, or explain a JavaScript-compatible pattern; applying a suggestion automatically switches that condition to Regex mode. By default it uses the primary model; set an optional **Regex Helper Model** in AI Settings to use a different model. Suggestions include flags, an explanation, warnings, and synthetic match/non-match examples. Nothing is changed automatically: review the suggestion, select **Apply Regex**, and then save the rule. The helper never reads mailbox content; it sends only the current regex, your request, and optional sample text that you enter.
